...
5. ``csv_concat.py``: satukan semua kategori menjadi ``all.csv``
6. ``streamlit_app.py``: aplikasi `Streamlit <https://streamlit.io>`_ sebagai
   web server untuk kenyamanan mencari melalui web browser
...
Jika menggunakan jenis `OpenAI <https://api.openai.com/v1>`_ seperti ChatGPT
maka buatlah file ``.env`` (dot env)::

    OPENAI_API_KEY=your-api-key

Bila menggunakan `OpenRouter <https://openrouter.ai>`_ maka menjadi::

    OPENAI_API_KEY=your-api-key
    OPENAI_URL=https://openrouter.ai/api/v1
    OPENAI_LLM_MODEL=poolside/laguna-xs-2.1:free
...
Jika ingin menyatukan seluruh kategori dalam sebuah file CSV::

    $ ~/env/bin/python csv_concat.py laptop hp mobo gpu storage psu server pc tablet watch mcu

Bagi pengguna Linux cukup jalankan::

    $ sh csv_concat_all.sh

Nanti akan terbentuk ``all.csv``. Kemudian jalankan::

    $ ~/env/bin/streamlit run streamlit_app.py
